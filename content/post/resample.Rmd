...
## 3. Demo
Get the climatological gridded rainfall dataset [PISCOpclim](http://ons.snirh.gob.pe/Peru/maproom/Monitoring/Meteorological/PISCO-Prec-v20.pdf) (~10 km) for [Arequipa](https://en.wikipedia.org/wiki/Arequipa_Region) and change the pixel size. [PISCOpclim](http://ons.snirh.gob.pe/Peru/maproom/Monitoring/Meteorological/PISCO-Prec-v20.pdf): Is based on the merging between the TRMM precipitation radar product and the peruvian rain gauge dataset.
...
```{r mypc, eval = TRUE}
message(sprintf('%s \n',system('lscpu',intern = T))) # only for linux users
```
